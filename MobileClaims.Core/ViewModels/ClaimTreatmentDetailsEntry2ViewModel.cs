...
{
    public class ClaimTreatmentDetailsEntry2ViewModel : ViewModelBase
    {
        private readonly IMvxMessenger _messenger;
        private readonly IClaimService _claimservice;
        private readonly MvxSubscriptionToken _claimsubmissionbenefitsretrieved;
        private readonly MvxSubscriptionToken _claimoptionsretrieved;
        private readonly MvxSubscriptionToken _shouldcloseself;

        public ClaimTreatmentDetailsEntry2ViewModel(IMvxMessenger messenger, IClaimService claimservice)
        {
            _messenger = messenger;
            _claimservice = claimservice;

            //TODO: this is taking too long to return
            _claimoptionsretrieved = _messenger.Subscribe<GetClaimOptionsComplete>((message) =>
            {
        ...
        private ClaimOption _lengthOfTreatment;
        public ClaimOption LengthOfTreatment
        {
            get
            {
        ...
        public DateTime DateOfTreatment
        {
            get
            {
                return (this._dateOfTreatment == DateTime.MinValue) ? DateTime.Now : this._dateOfTreatment;
            }
            set
            {
        ...
        public string TotalAmountOfVisit
        {
            get
            {
                return (String.IsNullOrEmpty(this._totalAmountOfVisit)) ? "" : this._totalAmountOfVisit;
            }
            set
            {
        ...
        private List<ClaimOption> _lengthsOfTreatment;
        public List<ClaimOption> LengthsOfTreatment
        {
            get
            {
                ...
                return new MvxCommand(() =>
                {
                    if (IsValid())
                    {
                        ...
                        TreatmentDetail td = new TreatmentDetail()
                        {
                            ...
                            TreatmentDuration = this.LengthOfTreatment,
                    ...
                    if (IsValid())
                    {
                        ...
                        foreach (TreatmentDetail td in _claimservice.Claim.TreatmentDetails)
                        {
                            if (td.ID == EditID)
                            {
                                ...
                                td.TreatmentDuration = this.LengthOfTreatment;
        ...
        public bool MissingDateOfTreatment
        {
            get
            {
                return _missingDateOfTreatment;
            }
            set
            {
        ...
        private bool IsValid()
        {
            ...
            BadValueAC = false;
            DateTooOld = false;
            TreatmentDetailEntry2Validator validator = new TreatmentDetailEntry2Validator();
            ValidationResult result = null;
            if (AmountPaidByAlternateCarrierVisible)
                result = validator.Validate<ClaimTreatmentDetailsEntry2ViewModel>(this, ruleSet: "default,AlternateCarrier");
            else
                result = validator.Validate<ClaimTreatmentDetailsEntry2ViewModel>(this, ruleSet: "default");
        ...
        private void PopulateFields()
        {
            ...
            if (td != null)
            {
                ...
                this.LengthOfTreatment = td.TreatmentDuration;
        ...
        {
            _messenger.Unsubscribe<GetClaimOptionsComplete>(_claimoptionsretrieved);
        ...
        {
            if (_claimservice.ClaimOptions != null)
            {
                Busy = false;
                LengthsOfTreatment = _claimservice.ClaimOptions;

                if (LengthsOfTreatment.Count > 0)
                    LengthOfTreatment = LengthsOfTreatment.Where(lt => lt.ID == "1.00").FirstOrDefault(); //default to 60 min
            }
            else
            {
                Busy = true;
