...
namespace DK.Ostebaronen.Droid.ViewPagerIndicator
{
    public class CirclePageIndicator : View, IPageIndicator
    {
        ...
        protected override void OnDraw(Canvas canvas)
        {
            ...
            if(null == _viewPager)
                return;
            var maxcount = _viewPager.Adapter.Count;
            int count;
            if (maxcount < 10)
                count = maxcount;
            else
            ...
            else
            {
                    ...
                    if (_orientation == (int)Orientation.Horizontal)
                    {
                        prevdX = longOffset + cx;
                        prevdY = shortOffset;
                    }
                    else
                    {
                        prevdX = shortOffset;
                        prevdY = longOffset + cx;
                    }
                }
                canvas.DrawCircle(prevdX, prevdY, _radius, _paintFill);
        ...
        private int MeasureLong(int measureSpec)
        {
            ...
            else
            {
                ...
                int count;
                if (_viewPager.Adapter.Count < 10)
                    count = _viewPager.Adapter.Count;
                else
                    count = 10;
